$(document).ready(function () {
    ...
    function retrievePlayer1() {
        return database.ref("Player 1").once('value').then(function (snapshot) {
            if (snapshot.exists()) {
                ...
                $("#player1-name").text(player1Name).prepend("Name: ");
                $("#player1-wins").text(player1Wins).prepend("Wins: ");
                $("#player1-losses").text(player1Losses).prepend("Losses: ");
            }
            else {
    ...
    function retrievePlayer2() {
        return database.ref("Player 2").once('value').then(function (snapshot) {
            if (snapshot.exists()) {
                ...
                $("#player2-name").text(player2Name).prepend("Name: ");
                $("#player2-wins").text(player2Wins).prepend("Wins: ");
                $("#player2-losses").text(player2Losses).prepend("Losses: ");
            }
            else {
    ...
    database.ref().child("Player 1").on("value", function (snapshot) {
        if (snapshot.exists()) {
            $("#player1-name").text(snapshot.val().name).prepend("Name: ");
            $("#player1-wins").text(snapshot.val().wins).prepend("Wins: ");
            $("#player1-losses").text(snapshot.val().losses).prepend("Losses: ");
        }
    })

    database.ref().child("Player 2").on("value", function (snapshot) {
        if (snapshot.exists()) {
            $("#player2-name").text(snapshot.val().name).prepend("Name: ");
            $("#player2-wins").text(snapshot.val().wins).prepend("Wins: ");
            $("#player2-losses").text(snapshot.val().losses).prepend("Losses: ");
        }
    })
